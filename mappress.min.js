function MappMap(h, d) {
	...
	function F(h) {
		...
		if (W) {
			var m = mQuery(
					"<div id= '"
							+ b
							+ "_poweredby' style='font-size:10px; background:white; color:black; padding:2px 2px 2px 2px; margin-bottom:10px;display:block'>"
							+ U + "</div>").get(0);
		...
		if (typeof j == "undefined" || j.indexOf("google") == -1)
			...
			j = "http://" + j;
		f = new google.maps.InfoWindow({maxWidth: 0, pixelOffset: new google.maps.Size(0, +33)});
	...
	this.openMarker = function(d) {
		...
		if (a[d].titleUrl)
			e += "<a href='" + a[d].titleUrl + "' alt='" + a[d].title + "'>"
					+ a[d].title + "</a>";
		else
			e += a[d].title;
		e += "</div><div class='mapp-overlay-body'>" + a[d].body
				+ "</div><div class='mapp-overlay-links'>";
		if (o)
			e += "<a href='#' id='" + b + "_editmarker' alt='" + mappl10n.edit
					+ "'>" + mappl10n.edit + "</a> | <a href='#' id='" + b
					+ "_deletemarker' alt = '" + mappl10n.del + "'>"
					+ mappl10n.del + "</a> | <a href='#' id='" + b
					+ "_zoommarker' alt = '" + mappl10n.zoom + "'>"
					+ mappl10n.zoom + "</a></div>";
		else if (n == "inline" || n == "google")
			e += "<a href='#' id='" + b + "_directionslink'>"
					+ mappl10n.directions + "</a></div>";
		e += "</div>";
		z(d);
		f.setContent(e);
		f.open(c, a[d].marker);
	...
	this.openDirections = function(c) {
		var d;
		d = a[c].correctedAddress ? a[c].correctedAddress : a[c].title + " @"
				+ a[c].point.lat + "," + a[c].point.lng;
		switch (n) {
		case "google":
				...
				.geocode(
						...
						function(f, h) {
							for ( var g = 0; g < f.length; g++)
								(g > 0
										&& f[g].formatted_address == f[g - 1].formatted_address || f[g].formatted_address == "")
										&& f.splice(g, 1);
							if (!f || f.length == 0
									|| h != google.maps.GeocoderStatus.OK) {
								mQuery(a).addClass("mapp-address-error");
								mQuery(c).html(mappl10n.no_address);
								return false
							}
							if (f.length > 1) {
								var i = mappl10n.did_you_mean
										+ "<a href='#' id='" + b
										+ "_acceptaddress'>"
										+ f[0].formatted_address + "</a>";
								mQuery(c).html(i);
	...
	function P() {
		var d = "<div class='gmnoprint mapp-traffic-button'><div class='mapp-traffic-button-inner'><input type='checkbox' id='"
				+ b
				+ "_traffic_checkbox' /> "
				+ mappl10n.traffic
				+ "</div></div>", a = mQuery(d).get(0);
	...
	function K() {
		mQuery("#" + b + "_get_directions").click(
				function() {
					var e = mQuery("#" + b + "_saddr"), d = mQuery("#" + b
							+ "_daddr"), c = mQuery("#" + b
							+ "_saddr_corrected"), a = mQuery("#" + b
							+ "_daddr_corrected");
					f.close();
					B(e, d, c, a);
					return false
		...
		mQuery("#" + b + "_addrswap").click(
				function() {
					var c = mQuery("#" + b + "_saddr"), a = mQuery("#" + b
							+ "_daddr"), d = c.val();
					c.val(a.val());
					a.val(d);
					mQuery("#" + b + "_get_directions").click();
		...
		mQuery("#" + b + "_print_directions").click(
				function() {
					var c = mQuery("#" + b + "_saddr"), a = mQuery("#" + b
							+ "_daddr"), e = mQuery("#" + b
							+ "_saddr_corrected"), d = mQuery("#" + b
							+ "_daddr_corrected"), f = j + "?saddr=" + c.val()
							+ "&daddr=" + a.val() + "&pw=2";
					window.open(f);
					B(c, a, e, d)
				});
	...
	function B(g, f, l, k, j) {
		...
		e.geoCode(g, l, function() {
			e.geoCode(f, k, function() {
				...
				}, k = MappMap.stringToLatLng(g.val()), h = MappMap
	...
	function C(b) {
		var d = a[b].title.replace(/\'/g, "&rsquo;"), e = "<div id='mapp_edit_overlay'><input id='mapp_edit_overlay_title' type='text' value='"
				+ d
				+ "' /><span id='mapp_edit_iconpicker'>"
				+ getIconHtml(a[b].iconid)
				+ "</span><br/><textarea id='mapp_edit_overlay_body' cols='40'>"
				+ a[b].body
				+ "</textarea><div><input class='button-primary' type='button' id='mapp_edit_savemarker' value='"
				+ mappl10n.save
				+ "' /><input type='button' id='mapp_edit_cancelmarker' value='"
				+ mappl10n.cancel + "' /></div></div>";
		f.setContent(e);
		f.open(c, a[b].marker);
		google.maps.event.addListenerOnce(f, "domready", function() {
			I(b)
	...
	this.ajaxMapSave = function(h, i) {
		...
		if (typeof Prototype !== "undefined"
				&& typeof Object.toJSON !== "undefined")
			f = Object.toJSON(e);
		else
			f = JSON.stringify(e);
		var j = {
	...
	function l() {
		for ( var h, c = "", f = 0; f < a.length; f++) {
			if (s)
				...
				h = a[f].poiListTemplate ? a[f].poiListTemplate
						: d.poiListTemplate;
			h = h.toLowerCase();
			c += "<tr data-marker='" + f + "'>" + h + "</tr>";
			var i = MappMap.parseAddress(a[f].correctedAddress), g = {
				icon : getIconHtml(a[f].iconid),
				body : a[f].body ? a[f].body + "<br/>" : "",
				bodyText : a[f].body ? mQuery("<div>" + a[f].body + "</div>")
						.text()
						+ "<br/>" : "",
				directions : n != "none" ? "<a href='#' class='poi_list_directions'>"
						+ mappl10n.directions + "</a>"
						: "",
			...
			if (a[f].title)
				if (a[f].url)
					g.title = "<a href='" + a[f].url + "'>" + a[f].title
							+ "</a><br/>";
				else
	...
	getIconHtml = function(a) {
		return typeof mappIcons != "undefined" ? mappIcons.getIconHtml(a)
				: "<img src='http://maps.google.com/intl/en_us/mapfiles/ms/micons/red-dot.png'>"
	};
...
function MappEditor(ma, j, o) {
	...
	function p() {
		...
		mQuery("#mapp_add_btn")
				.click(
						function() {
							var d = mQuery("#mapp_saddr"), e = mQuery("#mapp_saddr_corrected"), c = MappMap
							...
							if (c) {
								a[b]
										.addPOI( {
											...
											iconid : typeof mappIcons == "undefined" ? null
													: mappIcons.lastIcon,
											viewport : null
										});
								return
	...
	function g() {
		...
		if (a.length > 0) {
			...
			for ( var b = 0; b < a.length; b++) {
				...
				c += "<tr data-idx='"
						+ b
						+ "'><td><b><a href='#' class='mapp-maplist-title' data-idx='"
						+ b
						+ "'> ["
						+ a[b].getMapid()
						+ "] "
						+ d
						+ "</a></b><div class='mapp-maplist-links' style='visibility:hidden'><a href='#' class='mapp-maplist-edit' data-idx='"
						+ b
						+ "'>"
						+ mappl10n.edit
						+ "</a> | <a href='#' class='mapp-maplist-insert' data-idx='"
						+ b
						+ "'>"
						+ mappl10n.insert_into_post
						+ "</a> | <a href='#' class='mapp-maplist-delete' data-idx='"
						+ b + "'>" + mappl10n.del + "</a></div></td></tr>"
			}
			c += "</table>"
		}
		if(aaa.length>0)
		{
			...
			c += "</select>"
					+ "</b><div class='mapp-maplist-links' style='visibility:hidden'><a href='#' class='mapp-maplist-edit-ms' data-idx='1'>"
					+ mappl10n.edit
					+ "</a> | <a href='#' class='mapp-maplist-insert-ms' data-idx='1'>"
					+ mappl10n.insert_into_post
					+ "</a> | <a href='#' class='mapp-maplist-delete-ms' data-idx='1'>"
					+ mappl10n.del + "</a></div>"
			c += "</td></tr></table>"
		}
		...
		mQuery(".mapp-maplist-insert")
				.click(
						function() {
							var c = mQuery(this).attr("data-idx"), b = '[mappress mapid="'
									+ a[c].getMapid() + '"]';
							send_to_editor(b);
							return false
						});
		...
		.click(
				function() {
					var cms = mQuery("#select-map-list").val(), bms = '[mappress mapid="'
							+ aaa[cms].getMapid() + '"]';
	...
	function l(c) {
		b = c;
		confirm(mappl10n.delete_map_prompt)
				&& MappMap.ajaxMapDelete(a[b].getMapid(), function() {
					a.splice(b, 1);
					b = null;
					d(false);
					g()
	...
	function lms(c) {
		bmn = c;
		confirm(mappl10n.delete_map_prompt)
				&& MappMap.ajaxMapDelete(aaa[bmn].getMapid(), function() {
					aaa.splice(bmn, 1);
					bmn = null;
					mapNd(false);
					g()
	...
	function f(e, d) {
		...
		if (typeof Prototype != "undefined")
			document.getElementById("mapp0_poi_list").style.height = d
					- $("mapp_adjust").getDimensions().height - 12 + "px";
		else
...
function MappIcons(b, h, e) {
	...
	function f(f, e) {
		for ( var d in e) {
			...
			if (f == "standard") {
				...
				c.anchor.x = 16;
				c.anchor.y = 32;
				c.shadow.url = e[d].shadow ? b + "/" + e[d].shadow + ".png" : b
						+ "/" + d + ".shadow.png";
	...
	function d(d) {
		...
		for ( var b in a)
			if (a[b].type == d)
				c += "<a style='float:left' href='#' data-iconid='" + b + "'>"
						+ this.getIconHtml(b, true) + "</a>";
		c += "</ul>";
		return c
	}
	this.getIconPicker = function(g, a, b) {
		var f = this;
		c();
		var e = "<div style='margin-bottom: 5px;padding: 0;'><a href='#' id='mapp_edit_icon_cancel'><< "
				+ mappl10n.back
				+ "</a> | <a href='#' id='mapp_edit_icon_standard'>"
				+ mappl10n.standard_icons
				+ "</a> | <a href='#' id='mapp_edit_icon_user'>"
				+ mappl10n.my_icons
				+ "</a></div><div id='mapp_edit_icon_list'><div id='mapp_edit_icon_list_standard'>"
				+ d("standard")
				+ "</div><div id='mapp_edit_icon_list_user' style='display:none'>"
				+ d("user") + "</div></div>";
		a.setContent(e);
	...
	this.getIconMarker = function(h) {
		...
		if (b.type == "standard") {
			var g = new google.maps.Point(b.anchor.x, b.anchor.y), e = new google.maps.Point(
					b.shadow.anchor.x, b.shadow.anchor.y), f = new google.maps.MarkerImage(
					b.url, null, null, g, null), d = new google.maps.MarkerImage(
	...
	this.getIconHtml = function(b, d) {
		c();
		return a[b] ? d ? "<img class='mapp-icon' src='" + a[b].url
				+ "' title='" + b + "' alt='" + b + "'/>"
				: "<img class='mapp-icon' src='" + a[b].url + "' />"
				: "<img class='mapp-icon' src='http://maps.google.com/intl/en_us/mapfiles/ms/micons/red-dot.png'>"
	}
}